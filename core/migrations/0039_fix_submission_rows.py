...
class Migration:
    ...
    def forwards(self, orm):
        ek1 = orm.EthicsCommission.objects.get(name="Ethikkomission der Medizinischen Universität Wien")
        for submission_form in orm.SubmissionForm.objects.all():
            orm.Investigator.objects.create(
                submission=submission_form, 
                name="Univ. Doz. Dr. Ruth Ladenstein", 
                ethics_commission=ek1,
                subject_count=1,
                sign_date=datetime.date(2010,1,1),
            )
            
        orm.Submission.objects.update(ec_number='4321')
