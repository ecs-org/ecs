...
class Migration:
    ...
    def forwards(self, orm):
        flags = [
            ('yearly_report', u'Jahresbericht'), 
            ('final_report', u'Abschlussbericht'), 
            ('amendment_report', u'Ergänzungsreport'), 
            ('SAE_report', u'SAE Bericht'), 
            ('SUSAR_report', u'SUSAR Bericht')
        ]
        type_dict = {}
        for flag, name in flags:
            type_dict[flag] = orm.NotificationType.objects.create(name=name)
        #for flag, name in flags:
        #    orm.NotificationForm.objects.filter(type=None).filter(**{flag: True}).update(type=type_dict[flag])
    
    def backwards(self, orm):
        #orm.NotificationForm.objects.update(type=None)
        orm.NotificationType.objects.all().delete()
