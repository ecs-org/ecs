...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for submission in orm.Submission.objects.all():
            submission.current_submission_form = submission.forms.order_by('-pk')[0]
            submission.save()

        #orm.Vote.objects.update(submission_form=models.F('submission__current_submission_form'))
        for vote in orm.Vote.objects.all():
            vote.submission_form = vote.submission.current_submission_form
            vote.submission_form.current_pending_vote = vote
            vote.submission_form.save()
            vote.save()

    def backwards(self, orm):
        orm.Submission.objects.update(current_submission_form=None)
