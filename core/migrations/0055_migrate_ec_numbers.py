...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for s in orm.Submission.objects.all():
            try:
                year, num = s.ec_number.split("/")
                s.new_ec_number = int(year) * 10000 + int(num)
            except ValueError:
                try:
                    s.new_ec_number = 20100000 + int(s.ec_number)
                except ValueError:
                    pass
            s.save()
    ...
    def backwards(self, orm):
        for s in orm.Submission.objects.all():
            s.ec_number = "%s/%04d" % divmod(s.new_ec_number, 10000)
            s.save()
