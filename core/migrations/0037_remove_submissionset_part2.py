...
class Migration:
    ...
    def forwards(self, orm):
        ...
        for set in orm.SubmissionSet.objects.all():
            sform = set.submissionform
            sform.submission = set.submission
            sform.save()
    ...
    def backwards(self, orm):
        ...
        for set in orm.SubmissionSet.objects.all():
            set.delete()
        for sform in orm.SubmissionForm.objects.all():
            set = orm.SubmissionSet(submission=sform.submission, submissionform=sform)
            set.save()
            if sform.documents:
                set.documents = sform.documents.all()
                set.save()
