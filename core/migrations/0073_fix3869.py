...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for s in orm.Submission.objects.filter(forms__is_acknowledged=True):
            ack = s.forms.filter(is_acknowledged=True).order_by('-pk')[0]
            if not s.current_submission_form.pk == ack.pk:
                s.current_submission_form = ack
                s.save()
    ...
    models = {
        ...
        'core.temporaryauthorization': {
            ...
            'end': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 3, 6, 18, 12, 4, 257780)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
