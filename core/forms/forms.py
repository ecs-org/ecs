...
_queries = {
    ...
    'expedited':        lambda s,u: s.filter(is_expedited=True),
    'other':            lambda s,u: s.exclude(pk__in=s.amg().values('pk').query).exclude(pk__in=s.mpg().values('pk').query).exclude(pk__in=s.retrospective_thesis().values('pk').query).exclude(is_expedited=True),
    'b2':               lambda s,u: s.b2(),
    'b3':               lambda s,u: s.b3(),
    'b4':               lambda s,u: s.b4(),
