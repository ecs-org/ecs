...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # XXX: this migration could fail, when the docstash code changes,
        # but we can't use the model provided by south, because the model
        # methods are missing, e.g. docstash.transaction()
        from ecs.docstash.models import DocStash
        groups = ('ecs.core.views.submissions.create_submission_form', 'ecs.notifications.views.create_notification',)
        for docstash in DocStash.objects.filter(group__in=groups):
            with docstash.transaction():
                docstash['document_pks'] = [d.pk for d in docstash.get('documents', [])]
