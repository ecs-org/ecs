...
def sign_pdf_error(request):
    ...
    if request.REQUEST.has_key('cause'):
        ...
        cause = ''
    return HttpResponse(u'sign_pdf_error: error=[%s], cause=[%s]' % (error, cause))
...
def receive_pdf(request, jsessionid=''):
    if receive_pdf_is_authorized(request) is False:
        return HttpResponseForbidden('<h1>Access denied</h1>')
    # ..
    print 'receive_pdf jsessionid="%s"' % jsessionid
    ...
    return HttpResponse('receive_pdf got [%s]' % request)


def demo(request):
    url = request.build_absolute_uri('/pdfsigner/sign_pdf')
...
def sign_pdf(request):
    ...
    values = {
        ...
        'pdf-url': request.build_absolute_uri('/pdfsigner/send_pdf'), 
        'pdf-id': '1956507909008215134',
        'invoke-app-url': request.build_absolute_uri('/pdfsigner/receive_pdf'),
        'invoke-app-error-url': request.build_absolute_uri('/pdfsigner/sign_pdf_error'),
        # session-id=9085B85B364BEC31E7D38047FE54577D
        'locale': 'de',
    }
    data = urllib.urlencode(values)
    redirect = '%s?%s' % (url, data)
    print 'sign_pdf: redirect to [%s]' % redirect
