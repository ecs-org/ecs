...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        abbrevs = orm.ExpeditedReviewCategory.objects.values('abbrev').distinct().values_list('abbrev', flat=True)
        for abbrev in abbrevs:
            ercs = list(orm.ExpeditedReviewCategory.objects.filter(abbrev=abbrev))
            for erc in enumerate(ercs[1:]):
                erc.abbrev = '{0}_{1}'.format(abbrev, i)
                erc.save()
