...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        try:
            sentry_group = orm['auth.Group'].objects.get(name='sentryusers')
        except orm['auth.Group'].DoesNotExist:
            pass
        else:
            for user in sentry_group.user_set.all():
                user.ecs_profile.is_developer = True
                user.ecs_profile.save()
            sentry_group.permissions.all().delete()
            sentry_group.user_set = []
            sentry_group.save()
            sentry_group.delete()
    ...
    models = {
        ...
        'users.invitation': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'fd2996304c0945efa194dba22f8a4724'", 'unique': 'True', 'max_length': '32'})
        },
        'users.loginhistory': {
