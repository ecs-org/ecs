...
class diff_match_patch:
  ...
  def diff_wordsToChars(self, text1, text2):
    ...
    def find_whitespace(text, wordStart):
      for i in xrange(wordStart, len(text)):
        if text[i] in '\n\t ':
          return i
      return -1
    ...
    def diff_wordsToCharsMunge(text):
      ...
      while wordEnd < len(text) - 1:
        wordEnd = find_whitespace(text, wordStart)
        if wordEnd == -1:
          wordEnd = len(text) - 1
        ...
        if word in wordHash:
          chars.append(unichr(wordHash[word]))
        else:
          wordArray.append(word)
          wordHash[word] = len(wordArray) - 1
          chars.append(unichr(len(wordArray) - 1))
      return "".join(chars)
  ...
  def diff_charsToWords(self, diffs, wordArray):
    return self.diff_charsToLines(diffs, wordArray)
