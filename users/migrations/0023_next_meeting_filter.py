...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for s in orm['users.UserSettings'].objects.filter(user__groups__name=BOARD_MEMBER_GROUP):
            if not s.task_filter is None:
                data = {}
                for k, v in urlparse.parse_qs(s.task_filter).iteritems():
                    data[k] = v[0]
                data['next_meeting'] = u'on'
    ...
    models = {
        ...
        'users.invitation': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'ffa8de078906417099cf61ea9177d8d9'", 'unique': 'True', 'max_length': '32'})
        },
        'users.userprofile': {
