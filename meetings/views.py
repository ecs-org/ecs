...
from ecs.meetings.forms import (MeetingForm, TimetableEntryForm, FreeTimetableEntryForm, UserConstraintFormSet, 
    SubmissionReschedulingForm, AssignedMedicalCategoryForm, MeetingAssistantForm, RetrospectiveThesisVoteForm)
...
def meeting_assistant_retrospective_thesis(request, meeting_pk=None):
    from ecs.core.models.voting import FINAL_VOTE_RESULTS
    meeting = get_object_or_404(Meeting, pk=meeting_pk, started__isnull=False)
    form = RetrospectiveThesisVoteForm(request.POST or None, meeting=meeting)
    if form.is_valid():
        for submission in form.cleaned_data['submissions']:
            Vote.objects.create(submission_form=submission.current_submission_form, result='1')

    return render(request, 'meetings/assistant/retrospective_thesis.html', {
