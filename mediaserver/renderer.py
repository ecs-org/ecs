...
class Renderer(object):
    def renderPDFMontage(self, pdfdocument, width, tiles_x, tiles_y, aspect_ratio=1.41428, dpi=72, depth=8):
        margin_x = 0
        margin_y = 0
        ...
        for png_data in self._split_raw_pngstream(raw_pngstream):
            docshots.append(Docshot(tiles_x, tiles_y, width, pagenr, uuid=pdfdocument.uuid, data=png_data))
            pagenr = pagenr + 1
